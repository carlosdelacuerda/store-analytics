...
export default function DailyPage() {
  ...
  const { items: stockItems } = useAppSelector((s) => s.stock);
  ...
  async function handleSaleSave({
    ...
    const result = await dispatch(addSale({ date: record.date, type: saleModal.type, amountKes, items, saleItems }));
    if (addSale.fulfilled.match(result)) {
      dispatch(fetchStock());
  ...
  function renderSaleItems(saleItems: SaleItemDTO[], fallback: string | null) {
    if (saleItems.length === 0) return fallback;
    return saleItems.map((item) => `${item.quantity}x ${item.label}`).join(", ");
  }
                ...
                {record?.sales.map((sale) => (
                  <div key={sale.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2">
                    <div>
                      ...
                      {renderSaleItems(sale.saleItems, sale.items) && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeleteSale(sale.id)} className="text-gray-400 active:text-red-500" aria-label="Delete sale">✕</button>
                    </div>
                  </div>
                ))}
              </div>
